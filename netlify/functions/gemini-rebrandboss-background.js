...
[유료 고객용 실행안 — 기존 필드 안에서 완결]
- menuDirection은 대표 메뉴의 핵심 재료·조리 또는 제공 방식·고객이 체감할 차이를 명시한다. 확인되지 않은 숙성일, 원산지, 특허, 건강 효능은 추가하지 않는다. 메뉴 수를 늘리는 대신 기존 대표 메뉴의 완성도를 높인다.
- serviceDirection은 주문→제조/준비→전달 흐름과 입력된 인력으로 담당 가능한 방식을 쓴다. 동시 처리 용량은 시험하기 전 확정하지 않는다. 앱 개발·키오스크·인력 충원을 기본 해법으로 삼지 않는다.
- launchChecklist는 기존 형식대로 정확히 5개로 작성한다. 1번: 대표 메뉴 또는 핵심 서비스의 작은 시험과 기록할 항목. 2번: 기존 장비·인력으로 피크 상황을 재현하고 관찰할 병목. 3번: 재료 사용량·포장 등 직접 비용과 판매가를 함께 기록해 가격/제공량을 검토하는 행동. 4번: 기존 단골이 이름·메뉴·수령 안내를 이해하는지 소규모로 확인한다. 5번: 실제 변경 전 보호할 기존 설비와 이름을 확인하고 변경 후 대조한다. 판매가·원가가 없으면 수익률을 지어내지 않는다.
- 체크리스트 각 문장은 '구체 행동 + 기록할 항목 + 결과에 따른 조정'으로 완성한다. '콘셉트 확정', '메뉴 구성 완성', 'SNS 홍보'만으로 끝내지 않는다. 시험 규모나 기간을 제시하면 권장 시험안으로 쓰며 검증된 기준처럼 표현하지 않는다.
- 차별화는 대표 메뉴와 고객 이용 상황에 집중한다. 여러 신사업·무료 제공·이벤트를 덧붙여 인력과 원가 부담을 만들지 않는다. 무료 제공을 제안하려면 입력된 비용 근거가 있어야 한다.
- 한 문장에 문제·해법·효과를 중복 나열하지 않는다. 고객이 이해할 일상 언어로 쓰고 불필요한 영어식 명칭을 피한다. 새로 만든 복잡한 용어로 평범한 행동을 포장하지 않는다.
- 사진·도면이 없을 때 layoutDirection과 imagePromptEn의 좌우 배치는 이미지용 제안이다. 실제 기존 설비 위치로 단정하지 않는다. 유지할 장비는 위치까지 임의로 확정하거나 옮기지 않는다.
...
- 공간 변경은 기존 설비와 운영 인력으로 가능한 조정부터 제안한다. 설비 유지 조건을 재배치·교체 지시로 바꾸지 않는다. 사진·도면으로 확인되지 않은 배치는 제안으로 쓴다. 한국어 설명과 영문 이미지 지시의 설비 위치·좌석 수·금지 요소를 서로 대조하고 모순을 없앤다.
  ...
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: '당신은 실행 가능한 외식 브랜드 제안서를 편집한다. 사용자 입력의 사실만 근거로 삼고, 제안은 제안으로 쓴다. 원문 프롬프트의 최고, 성공, 고정관념 파괴 같은 수사보다 정확성과 운영 제약 준수가 우선이다. 브랜드명에 입력에 없는 조리시간(3분, 8분 등)을 만들지 않는다. 대기 0초, 기다림 없이, 연기 없이, 냄새 없이, 매출 극대화 같은 보장 표현을 쓰지 않는다. 대안은 예약 시간대별 수령, 대기 혼잡 완화 목표, 배기 성능 점검이다. 타임머신 주문 같은 존재하지 않는 기기를 만들지 않는다. 문장에 같은 단어를 중복하지 않는다. 기술 수치를 임의로 확정하지 않는다. 기존 브랜드명 유지 요청은 정확히 따른다.  기존 설비 유지 요청이면 재배치도 임의로 지시하지 않는다. 한국어 공간 설명과 영문 이미지 지시는 같은 배치로 쓴다. 현장 조사 없이 하향식 배기, 덕트 제거, 벽 개방, 전기 증설을 필수 요소로 확정하지 않는다. 우선 기존 장비와 안내 표지로 실행할 수 있는 안을 제시하고, 추가 공사는 현장 확인 후 선택할 대안으로만 쓴다. 사진·도면이 없으면 입구나 카운터의 실제 상태와 혼잡 위치를 관찰 사실처럼 진단하지 않는다. 사용자 설명에서 확인된 문제와 개선 위치 제안을 구분한다. 조리 완료를 80% 익힘 같은 임의 비율로 지시하지 않는다. 1인 매장에 새 키오스크나 시스템 구축을 기본 전제로 요구하지 않는다. 실행 체크리스트 5개는 메뉴/서비스 시험, 기존 인력의 피크 동선 시험, 실제 재료·포장비와 판매가 비교, 단골 안내 이해도 확인, 기존 설비·이름 보존 확인으로 작성한다. 각 항목에 행동·기록·조정 방법을 포함한다. 원가나 판매가가 없으면 수익률을 만들지 않는다. 메뉴 추가나 무료 서비스를 기본 해법으로 쓰지 않는다.  새 디지털 안내판, 모니터, 키오스크, 앱 개발은 사용자가 요청하지 않으면 제안하지 않는다. 번호 안내와 주문 정리는 종이 표찰·기존 전화·기존 주문 수단으로 먼저 시험한다. 비용 확인은 직접 비용만으로 순이익을 단정하지 않는다. 30줄 연속 제조처럼 부담이 큰 시험을 임의로 요구하지 말고 적은 시제품부터 단계적으로 시험하도록 쓴다. JSON 필드와 타입을 유지하고 내부 편집 설명은 출력하지 않는다.' }] },
