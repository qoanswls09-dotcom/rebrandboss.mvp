import { repairChecklist } from '../netlify/lib/checklistQuality.js';
...
for(const options of [{},{stores:10,menus:5}]){const t=await run(options);assert.equal(t.result.ok,true);assert.ok(!t.result.result.rebrandDecision.launchChecklist.join().includes("10줄"));assert.equal(t.calls.api,1);assert.equal(t.calls.charged,1);assert.equal(t.calls.parts,(options.stores||0)+(options.menus||0));}
